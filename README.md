...
- 系统配置
  ...
  - AI系统提示词配置（热更新，显示自定义/代码默认标签，支持恢复代码默认值）
  - AI知识库管理（43条FAQ，AI智能建议从用户聊天记录自动提取候选条目）
...
- **多模型协同**：Agent循环中按场景路由（i=0推理模型分析意图，i>0对话模型执行工具），图片消息路由到视觉模型
- **视觉模型支持**：`convertMessagesForVision`方法将`[图片: name](url)`转为base64多模态格式发送给视觉模型，AI可真正分析截图内容
- **DeepSeek大模型接入**：基于DeepSeek-V4-Flash，支持多轮对话和上下文记忆
- **System Prompt增强**：结构化7大模块（思考优先/核心职责/可用工具/工具调用规则/图片分析规则/平台导航回答规则/安全约束），强制AI先分析再回答
- **平台知识注入**：系统提示词中注入平台规则知识（密码重置、商品审核、个人中心5标签页、收货地址入口、资金流水查看方式等），**始终注入**确保AI任何时候都能依据平台知识回答导航问题
- **FAQ知识库**：43条FAQ覆盖全平台功能，embedding向量语义检索（4096维）+ TF-IDF降级，Top3注入prompt
- **AI智能FAQ建议**：从用户聊天记录自动提取FAQ候选条目，管理员审核后加入知识库
- **模板响应**：常见问题直接返回预定义回复，不调用API（0.1s响应）
...
- **引用功能**：引用内容独立显示在输入框上方（单行+省略号+X删除），消息中引用特殊展示为独立块
- **粘贴图片**：支持Ctrl+V粘贴剪贴板图片自动上传
