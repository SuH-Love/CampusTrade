    ...
    <el-table :data="orders" stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'UNPAID' && activeTab === 'buyer'" type="primary" size="small" @click="handlePay(row.id)">支付</el-button>
          <el-button v-if="row.status === 'UNPAID' && activeTab === 'buyer'" size="small" @click="handleCancel(row.id)">取消</el-button>
          <el-button v-if="row.status === 'PAID' && activeTab === 'seller'" type="primary" size="small" @click="handleShip(row.id)">发货</el-button>
          <el-button v-if="row.status === 'SHIPPED' && activeTab === 'buyer'" type="success" size="small" @click="handleFinish(row.id)">确认收货</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const statusTagType = (status: string) => {
  const map: Record<string, string> = { UNPAID: 'warning', PAID: 'primary', SHIPPED: '', FINISHED: 'success', CANCELLED: 'info', REFUNDING: 'danger', REFUNDED: 'info' }
  return map[status] || ''
}
