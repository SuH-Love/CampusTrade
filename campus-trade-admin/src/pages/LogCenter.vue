...
const operationLabel = (op: string) => {
  const map: Record<string, string> = {
    ...
    favoriteGoods: '收藏商品', unfavoriteGoods: '取消收藏', listFavorites: '收藏列表', getMyGoods: '我的商品',
    ...
    listBuyerOrders: '买家订单', listSellerOrders: '卖家订单',
    ...
    listActiveBanners: '轮播图列表', getActiveBanners: '活跃轮播图',
    createBanner: '创建轮播图', updateBanner: '编辑轮播图',
    deleteBanner: '删除轮播图', toggleBannerStatus: '切换轮播图状态',
    getBannerList: '轮播图列表', toggleBanner: '切换轮播图',
    listAll: '分类列表', deleteImage: '删除图片', uploadImage: '上传图片',
    toggleFollow: '关注/取关', isFollowing: '是否关注', getFollowCounts: '关注数',
    getFollowing: '关注列表', getFollowers: '粉丝列表',
    getAverageRating: '卖家评分', getCategoryList: '分类列表',
    orderTimeout: '订单超时取消', orderTimeoutCheck: '订单超时检查', 订单创建: '订单创建'
  }
  return map[op] || op
}
